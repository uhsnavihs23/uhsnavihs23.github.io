...
for filepath in html_files:
    ...
    # Regex to find everything from /* GO RETRO BETA FEATURE to the end of the style block, but before </style>
    if '/* GO RETRO BETA FEATURE' in content:
        # We replace the entire retro and dark aero CSS
        content = re.sub(r'/\*\s*GO RETRO BETA FEATURE\s*\*/.*?(?=\s*</style>)', xp_retro_css, content, flags=re.DOTALL)
        
        with open(filepath, 'w') as f:
            f.write(content)

print("Windows XP / IE6 CSS injected!")
