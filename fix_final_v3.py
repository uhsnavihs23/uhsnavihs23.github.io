...
movie_css_addition = """
    /* Fix Movie Recommender dynamic text colors in Dark Mode */
    ...
    html.dark .text-black { color: #ffffff !important; }
    
    html.dark.retro .text-blue-800, html.dark.retro .text-blue-700, html.dark.retro .text-blue-600 { color: #ffff00 !important; }
"""
...
for filepath in html_files:
    ...
    # 2. Add or replace Dark CSS Retro block
    if "DARK THEME RETRO MODE CONSISTENCY" in content:
        content = re.sub(r'/\* DARK THEME RETRO MODE CONSISTENCY.*?\n    \}\n', dark_css_replacement, content, flags=re.DOTALL)
    else:
        # Fallback if not there
        content = re.sub(r'(html\.retro button:active \{ border-style: inset !important; \})', r'\1\n' + dark_css_replacement, content)
        
    # 3. If movie-recommender, inject the text-blue fix
    if "movie-recommender" in filepath and "index.html" in filepath:
        if "Fix Movie Recommender dynamic text colors" not in content:
            content = content.replace("/* ANY ELEMENT WITH WHITE BG IN DARK MODE */", movie_css_addition + "\n    /* ANY ELEMENT WITH WHITE BG IN DARK MODE */")

    with open(filepath, 'w') as f:
        f.write(content)

print("Final robust fixes injected universally.")
