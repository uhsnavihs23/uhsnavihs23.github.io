...
unified_js = """
    <script>
        ...
        function initRetroMode() {
            ...
            // The retroToggle button might be created dynamically or exist in HTML
            ...
            window.setRetro = function(isRetro) {
                if (isRetro) {
                    ...
                    if (!document.getElementById('ie-chrome-top')) {
                        ...
                        const rootLink = window.location.origin; // better for gh pages
                        
                        // Using raw Unicode to prevent corrupted image links
                        chromeTop.innerHTML = `
            ...
            // Re-bind all retroToggle buttons just in case
...
for filepath in html_files:
    ...
    # 1. Clean out ANY old `setRetro` logic blocks floating around.
    # We will just obliterate anything matching `const retroToggle = ... setRetro(false); }`
    content = re.sub(r"const retroToggle = document.getElementById\('retroToggle'\);.*?if \(retroToggle\) \{ retroToggle\.addEventListener\('click', \(\) => \{ setRetro\(!html\.classList\.contains\('retro'\)\); \}\); \}", "", content, flags=re.DOTALL)
    
    # Clean up empty script tags if they were left
    content = re.sub(r"<script>\s*</script>", "", content, flags=re.DOTALL)
    
    # 2. Inject the new robust `unified_js` right before </body>
    # First, make sure we aren't injecting it twice
...
print("Unified global retro JS injected into all files!")
